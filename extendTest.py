...
from datasets.extendDataSet import clfDataSet
...
def test(model, test_loader, device):
    ...
    with torch.no_grad():
        for idx, (imgs, labels, imgs_name_) in tqdm(enumerate(test_loader), desc='Train', total=len(test_loader)):
    ...
    np.save('./output/pred_extend.npy', argmax_output_list)
    np.save('./output/true_extend.npy', labels_list)
    return fpr, tpr, thresholds, roc_auc_micro, total_accuracy, total_f1_score_macro, total_f1_score_mirco, total_cm


def main(args):

    device, num_classes, batch_size,  model_name =\
         args.device, args.num_classes, args.batch_size, args.model_name
    pth, save_clf, test_data_dir = \
         args.pth, args.save_clf, args.test_data_dir
    ...
    test_data_loader = DataLoader(
        dataset=clfDataSet(test_data_dir), batch_size=batch_size,
